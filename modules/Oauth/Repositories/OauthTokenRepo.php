...
use Modules\Oauth\Exceptions\OauthRootError;
use Modules\Oauth\Models\OauthTokenModel;
...
class OauthTokenRepo extends BaseRepository
{
    ...
    public function queryMatchCollect(QueryMatch $QM)
    {
        ...
        $QM->incModelHaveClosure(
            OauthTokenModel::class, $includeArr, function ($include) use (&$query) {
            $query->with($include);
        }
        );
    ...
    //根据 $userMark 标记,获取未过期的 token
    public static function isClientTokenHave($userMark, $scopeId, $clientId)
    {
        ...
        $field  = array_merge($field, array_keys($where));
        $result = self::$query->select($field)
            ->where($where)->where('expire_time', '>', date('Y-m-d H:i:s'))
            ->first();
        return $result;
    }

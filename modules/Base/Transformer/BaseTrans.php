...
namespace Modules\Base\Transformer;
...
class BaseTrans
{
    ...
    /*
     * notes: 单例
     * @author 陈鸿扬 | @date 2021/3/9 12:30
     */
...
//v扩展函数区
    ...
    {
        //数据对象 转 数组
        if (gettype($data) == 'object' && gettype($data[0]) == 'object') {
            $newCollect = [];
...
//v退休函数区
     ...
     * @param bool $force
     * @param null $recursionIncludePrefix
     * @return bool
     * @author 陈鸿扬 | @date 2022/5/18 13:58
     * @deprecated 改用新函数 $this->belongsTo()
     */
    public function includeBelongsTo($includeSign, $model, $fields, $transformerClass, $force = false, $recursionIncludePrefix = null)
    {
        $includes = request()->get('_include');
        $includeArr = explode(',', $includes);

        if (in_array($includeSign, $includeArr) && !empty($model)) {
     ...
     * @return array|bool
     * @author 陈鸿扬 | @date 2022/5/18 13:58
     * @deprecated 改用新函数 $this->hasMany()
     */
    public function includeHasMany($includeSign, $models, $fields, $transformerClass, $force = false, $recursionIncludePrefix = null)
    {

        $includes = request()->get('_include');
        $includeArr = explode(',', $includes);

        if (in_array($includeSign, $includeArr) && !empty($models)) {
            $objects = [];

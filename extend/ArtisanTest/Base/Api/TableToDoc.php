...
use Extend\ArtisanTest\Base\Lib\PdoDB;
...
class TableToDoc
{
    ...
    public function __construct($moduleName,$childName,$action="table",$testPath=null){

        if( !empty($testPath) ){ $this->testPath =  $testPath; }
        else{ $this->testPath =  base_path().'test'; }
    ...
    function DB($childName,$type=null){
        $opt = config("database.connections.mysql");

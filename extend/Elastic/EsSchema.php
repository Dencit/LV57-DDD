...
class EsSchema{
    ...
    protected function __construct( $settings = null, $option = null )
    {
        //节点-默认值
        //副本节点数 - 默认单节点
        if( !isset($settings['number_of_replicas']) ){ $settings['number_of_replicas'] = 0; }
        //索引分片数 - 默认5个
        if( !isset($settings['number_of_shards']) ){ $settings['number_of_shards'] = 5; }
        //索引刷新间隔 - 大量导入时,可设置为-1,以加快导入速度 - 默认重置为系统设置
        if( !isset($settings['refresh_interval']) ){ $settings['refresh_interval'] = null; }
        //
        if( !empty($settings) ){ self::$settings = $settings; }

        //索引-默认值
        //
        if( !isset($option['_all']) ){ self::$option ['_all']=['enabled'=>false]; }
        //
        if( !isset($option['_source']) ){ self::$option ['_source']=['enabled' => true]; }
        //
        if( !isset($option['_routing']) ){ self::$option ['_routing']=['required' => true]; }
        //
        if(!empty($option)){
            self::$option = $option;
    ...
    public function setting($settings = null)
    {
        ...
        //副本节点数 - 默认单节点
        if( isset($settings['number_of_replicas']) ){ self::$settings['number_of_replicas'] = $settings['number_of_replicas']; }
        //索引分片数 - 默认5个
        if( isset($settings['number_of_shards']) ){  self::$settings['number_of_shards'] = $settings['number_of_shards']; }
        //索引刷新间隔 - 大量导入时,可设置为-1,以加快导入速度 - 默认重置为系统设置
        if( isset($settings['refresh_interval']) ){  self::$settings['refresh_interval'] = $settings['refresh_interval']; }

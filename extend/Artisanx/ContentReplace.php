...
class ContentReplace
{
    ...
    public function forHiddenTag(&$content){
        $r = $this->r; $n = $this->n; $t = $this->t; $s = $this->s;  $space=$r.$n.$s.$s.$s.$s;
        $start = ''; $end = '';
        if (!$this->opt['debug']) {
            $start = "/*"; $end = "*/";
    ...
    public function forCodeBlockTag(&$content){ $r = $this->r; $n = $this->n; $t = $this->t; $s = $this->s;
        //指定保留的代码块
    ...
    public function forRules(&$content){ $r = $this->r;  $n = $this->n;  $t = $this->t;  $s = $this->s;
        $childName = ($this->childName);
        $repValidate = '';  $space = $r . $n . $t . $t;
        ...
        if ($res) {
            foreach ($res as $k => $v) {
                ...
                switch ($typeArr[0]) {
                    default : break;
                    case "int": $ruleStr = 'integer|gt:0' . '|between:0,' . $typeArr[1]; break;
                    case "decimal": $ruleStr = 'numeric|gt:0' . '|between:0,' . $typeArr[1];  break;
                    case "string":  $ruleStr = 'string' . '|between:0,' . $typeArr[1];    break;
                    case "text":    $ruleStr = 'string';   break;
                    case "date_time":   $ruleStr = 'date';  break;
                };
                switch ($field) {
                    default :   break;
                    case "status":  $ruleStr = 'integer|in:1,2';    break;
                    case "type":    $ruleStr = 'integer|in:1,2';    break;
                    case "date":    $ruleStr = 'date_format:Y-m-d';  break;
                    case "phone":   $ruleStr = 'number|max:11'; break;
                    case "sex": $ruleStr = 'integer|in:0,1,2';  break;
                    case "ip":  $ruleStr = 'ip';    break;
                };
    ...
    public function forMessages(&$content){   $r = $this->r; $n = $this->n; $t = $this->t; $s = $this->s;
        $childName = ($this->childName);
        $repValidate = '';
        $space = $r . $n . $t . $t;
        $res = $this->DB($childName, "showFullColumns");
    ...
    public function forFields(&$content){ $r = $this->r; $n = $this->n; $t = $this->t; $s = $this->s;
        $childName = ($this->childName);
        $repInfo = ''; $space = $r . $n . $t . $t;
    ...
    public function forFillAble( &$content ){  $r = $this->r; $n = $this->n; $t = $this->t; $s = $this->s;
        $childName = ($this->childName);
        ...
        if($res){
            foreach ($res as $k=>$v){
                switch ($v['name']){
                    case "id":
                        case "created_at": case"updated_at": case"deleted_at":
                        case "create_time": case"update_time": case"delete_time":case"is_delete":
                    break;
                    default:
                        if( $k<count($res)-1 ){
    ...
    public function forGuarded( &$content ){  $r = $this->r; $n = $this->n; $t = $this->t; $s = $this->s;
        $childName = ($this->childName);
        $res = $this->DB($childName,"getTableFields");
        $repInfo=''; $space=$r . $n . $t . $t;
        if($res){
            foreach ($res as $k=>$v){
                switch ($v['name']){
                    case "id": case "created_at": case "create_time":
                    if( $k<count($res)-1 ){
                        $repInfo .= "\"".$v['name']."\",".$space;
                    }else{
                        $repInfo .= "\"".$v['name']."\",".$space;
    ...
    public function forCasts( &$content ){  $r = $this->r; $n = $this->n; $t = $this->t; $s = $this->s;
        $childName = ($this->childName);
        $res = $this->DB($childName,"getTableFields");
        $repInfo=''; $space=$r . $n . $t . $t;
        if($res){
            foreach ($res as $k=>$v) {
                ...
                switch ($v['native_type']){
                    default :  break;
                    case "LONG": $v['native_type']='integer'; break;
                    case "LONGLONG": $v['native_type']='integer'; break;
                    case"TINY": $v['native_type']='integer'; break;
                    case "VAR_STRING": $v['native_type']='string'; break;
                    case "BLOB": $v['native_type']='boolean'; break;
                    case "TEXT": $v['native_type']='string'; break;
                    case "DATETIME":  case "TIMESTAMP": $v['native_type']='datetime'; break;
                    case "NEWDECIMAL": $v['native_type']='float'; break;
                };
                switch ($v['name']){
                    default :  break;
                    case "status": $v['native_type']='integer'; break;
                    case "type": $v['native_type']='integer'; break;
                };
                if ($k < count($res) - 1) {
    ...
    public function forData( &$content ){ $r = $this->r; $n = $this->n; $t = $this->t; $s = $this->s;
        $childName = ($this->childName);
        $res = $this->DB($childName,"getTableFields");
        $repInfo=''; $space=$r.$n.$t.$t.$t;
        if($res){
            foreach ($res as $k=>$v) {
                //var_dump( $v['native_type'] );
                switch ($v['native_type']){
                    default : $v['native_type']='(string)'; break;
                    case "LONGLONG": case "LONG": case"TINY":
                    $v['native_type']='(int)'; break;
                    case "VAR_STRING":
                        $v['native_type']='(string)'; break;
                    case "DATETIME":  case "TIMESTAMP":
                    $v['native_type']='(string)'; break;
                    case "NEWDECIMAL":
                        $v['native_type']='(float)'; break;
                };
    ...
    public function forTestInData( &$content ){ $r = $this->r; $n = $this->n; $t = $this->t; $s = $this->s;
        $childName = ($this->childName);
        $res = $this->DB($childName,"getTableFields");
        $repInfo=''; $space=$r.$n.$t.$t.$t;
        if($res){
            foreach ($res as $k=>$v) {
                //var_dump( $v['native_type'] );
                switch ($v['native_type']){
                    default : $v['native_type'] = "'".$v['name']."'"; break;
                    case "LONGLONG": case "LONG": case"TINY":
                    $v['native_type']= 0 ; break;
                    case "VAR_STRING":
                        $v['native_type']="'".$v['name']."'"; break;
                    case "DATETIME":
                    $v['native_type']= "'".date('Y-m-d H:i:s',time())."'"; break;
                    case "TIMESTAMP":
                    $v['native_type']= time(); break;
                    case "NEWDECIMAL":
                        $v['native_type']=0.0; break;
                };
                ...
                switch ($v['name']) {
                    default:
                        $repInfo .= "\"".$v['name']."\"=>".$v['native_type']."," .$space;
                        break;
                    case "mobile": $repInfo .= "\"".$v['name']."\"=> '18500000000' ," .$space; break;
                    case "photo": $repInfo .= "\"".$v['name']."\"=> '/none',".$space; break;
                    case "update_time": case "delete_time": break;
                }
    ...
    public function forTestUpData( &$content ){ $r = $this->r; $n = $this->n; $t = $this->t; $s = $this->s;
        $childName = ($this->childName);
        $res = $this->DB($childName,"getTableFields");
        $repInfo=''; $space=$r.$n.$t.$t.$t;
        if($res){
    ...
    public function forTestInList( &$content ){ $r = $this->r; $n = $this->n; $t = $this->t; $s = $this->s;
        $childName = ($this->childName);
        $res = $this->DB($childName,"getTableFields");
        $repInfo=''; $space=$r.$n.$t.$t.$t;
        if($res){
            foreach ($res as $k=>$v) {
                ...
                switch ($v['name']) {
                    default:
                        ...
                        break;
                    case "mobile": $repInfo .= "\"".$v['name']."\"=> '18500000000' ," ; break;
                    case "photo": $repInfo .= "\"".$v['name']."\"=> '/none'," ; break;
                    case "create_time": $repInfo .= "\"".$v['name']."\"=> \$createTime," ; break;
                    case "update_time": case "delete_time": break;
                }
    ...
    function DB($childName,$type=null){ $opt = $this->opt;
    ...
    function childNameFilter($childName){
        ...
        for ($i=1;$i<10;$i++){
            ...
            else{ $newChildName=preg_replace("/_$/",'',$newChildName);
                break;
            }
        }
    ...
    function childNameFirstFilter($childName){
        $newChildName=''; $first=0;
        for ($i=1;$i<10;$i++){
            preg_match("/([A-Z]{1}[a-z 0-9]+){".$i."}/",$childName,$m);
            if(!empty($m[1])){ $first++;
                if($first<2){  $newChildName.= strtolower($m[1]);
                }else{ $newChildName.= $m[1]; }
            }
        }
        return $newChildName;
    }
    ...
    function childNameDashFilter($childName){
        $newChildName=''; $first=0; $leng = strlen($childName);
        for ($i=1;$i<=$leng;$i++){
            preg_match("/([A-Z]{1}[a-z 0-9]+){".$i."}/",$childName,$m);
            if(!empty($m[1])){ $first++;
                if($first<2){  $newChildName = strtolower($m[1]); }
                else{ $newChildName.= '-'.strtolower($m[1]); }
            }
        }
        return $newChildName;
    }

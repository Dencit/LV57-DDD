...
//#########################################################
    ...
    function setModuleProviderFile($moduleName, $childName)
    {
        ...
        if ($isExistFile) {
            ...
            $msg = "Created : Module " . $moduleName . ' | SetModuleServiceProvider | OK';
            $this->console($msg, "yellow");
            return true;
        }
        return false;
    ...
    function setRouteProviderFile($moduleName, $childName)
    {
        ...
        if ($isExistFile) {
            ...
            $msg = "Created : Module " . $moduleName . ' | SetRouteServiceProvider | OK';
            $this->console($msg, "yellow");
            return true;
        }
        return false;
    ...
    function setJobFile($moduleName, $childName, $dbOn = null)
    {
        ...
        if ($isExistFile) {
            ...
            $msg = "Created : Module " . $moduleName . ' | SetJobFile | OK';
            $this->console($msg, "yellow");
            return true;
        }
        return false;
    ...
    function setRequestFile($moduleName, $childName, $dbOn = null, $dbOptStr = null)
    {
        ...
        $isExistFile    = file_exists($option['put_path']);
        if ($isExistFile) {
            $msg = "Exception : " . $moduleName . " | SetValidateFile | is exists !";
            $this->console($msg, "red");
        } else {
            ...
            $msg = "Created : Module " . $moduleName . ' | SetValidateFile | OK';
            $this->console($msg, "yellow");
            return true;
        }
        return false;
    ...
    function setRepositoryFile($moduleName, $childName, $dbOn = null, $dbOptStr = null)
    {
        ...
        $root           = $this->opt['root_path'];
        $option         = [
            "get_path"    => $root . "\\" . $moduleDomeName . "\\Repositories\\" . ucwords($childDemoName) . "Repo.php",
            "module_name" => $moduleName,
            "child_name"  => $childName,
            "put_path"    => $root . "\\" . $moduleName . "\\Repositories\\" . ucwords($childName) . "Repo.php",
        ];
        $isExistFile    = file_exists($option['put_path']);
        if ($isExistFile) {
    ...
    function setTransformerFile($moduleName, $childName, $dbOn = null, $dbOptStr = null)
    {
        ...
        $root           = $this->opt['root_path'];
        $option         = [
            "get_path"    => $root . "\\" . $moduleDomeName . "\\Http\\Transformers\\" . ucwords($childDemoName) . "Trans.php",
            "module_name" => $moduleName,
            "child_name"  => $childName,
            "put_path"    => $root . "\\" . $moduleName . "\\Http\\Transformers\\" . ucwords($childName) . "Trans.php",
        ];
        $isExistFile    = file_exists($option['put_path']);
        if ($isExistFile) {
            ...
            $msg = "Created : Module " . $moduleName . ' | SetTransformerFile | OK';
            $this->console($msg, "yellow");
            return true;
        }
        return false;
    ...
    function setServiceFile($moduleName, $childName, $dbOn = null, $dbOptStr = null)
    {
        ...
        $root           = $this->opt['root_path'];
        $option         = [
            "get_path"    => $root . "\\" . $moduleDomeName . "\\Services\\" . ucwords($childDemoName) . "Service.php",
            "module_name" => $moduleName,
            "child_name"  => $childName,
            "put_path"    => $root . "\\" . $moduleName . "\\Services\\" . ucwords($childName) . "Service.php",
        ];
        $isExistFile    = file_exists($option['put_path']);
        if ($isExistFile) {
            $this->fileEditChange($option, function ($option, $demoContent, $changeContent) use ($opt, $dbOn, $dbOptStr) {
            ...
            $msg = "Created : Module " . $moduleName . ' | SetServiceFile | OK';
            $this->console($msg, "yellow");
            return true;
        }
        return false;
    ...
    function setLogicFile($moduleName, $childName, $dbOn = null, $dbOptStr = null)
    {
        ...
        $root        = $this->opt['root_path'];
        $option         = [
            "get_path"    => $root . "\\" . $moduleDomeName . "\\Http\\Logics\\" . ucwords($childDemoName) . "Logic.php",
            "module_name" => $moduleName,
            "child_name"  => $childName,
            "put_path"    => $root . "\\" . $moduleName . "\\Http\\Logics\\" . ucwords($childName) . "Logic.php",
        ];
        $isExistFile    = file_exists($option['put_path']);
        if ($isExistFile) {
            $this->fileEditChange($option, function ($option, $demoContent, $changeContent) use ($opt, $dbOn, $dbOptStr) {
    ...
    function setEnumFile($moduleName, $childName, $dbOn = null)
    {
        ...
        if ($isExistFile) {
            ...
            $msg = "Created : Module " . $moduleName . ' | SetStatusFile | OK';
            $this->console($msg, "yellow");
            return true;
        }
        return false;
    ...
    function setExceptionFile($moduleName, $childName, $dbOn = null)
    {
        $moduleDomeName = $this->moduleDomeName;
        $childDemoName  = $this->childDemoName;
        $root           = $this->opt['root_path'];
        $option         = [
            "get_path"    => $root . "\\" . $moduleDomeName . "\\Exceptions\\" . ucwords($moduleDomeName) . "RootError.php",
            "module_name" => $moduleName,
            "child_name"  => $childName,
            "put_path"    => $root . "\\" . $moduleName . "\\Exceptions\\" . ucwords($moduleName) . "RootError.php",
        ];
        $isExistFile    = file_exists($option['put_path']);
        if ($isExistFile) {
            ...
            $msg = "Created : Module " . $moduleName . ' | SetErrorCodeFile | OK';
            $this->console($msg, "yellow");
            return true;
        }
        return false;
    ...
    function setApiRoute($moduleName, $childName, $dbOn = null)
    {
        ...
        //配置路径
        $option = [
            "get_path"    => $root . "\\" . $moduleDomeName . "\\Routes\\api.php",
            "module_name" => $moduleName,
            "child_name"  => $childName,
            "put_path"    => $root . "\\" . $moduleName . "\\Routes\\api.php",
        ];
    ...
    function changeModelFields($moduleName, $childName, $dbOptStr = null)
    {
        ...
        $root           = $this->opt['root_path'];
        $option         = [
            "get_path"    => $root . "\\" . $moduleName . "\\Models\\" . ucwords($childName) . "Model.php",
            "module_name" => $moduleName,
            "child_name"  => $childName,
            "put_path"    => $root . "\\" . $moduleName . "\\Models\\" . ucwords($childName) . "Model.php",
        ];
    ...
    function changeTransformerFields($moduleName, $childName, $dbOptStr = null)
    {
        ...
        if (!$isExistFile) {
            $msg = "Exception : " . $moduleName . " is exists !";
            $this->console($msg, "red");
        } else {
            $this->fileEditCopy($option, function ($option, $content) use ($opt, $dbOptStr) {
                //修改默认数据库连接配置,用于跨库查询
    ...
    {
        $childPathArr = ['Config', 'Consoles', 'Controllers', 'Database', 'Exceptions', 'Models', 'Providers', 'Routes', 'Services', 'Transformers', 'Requests'];

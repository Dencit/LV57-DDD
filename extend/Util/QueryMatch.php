...
class QueryMatch
{
    ...
    public static function instance(array $requestQuery)
    {
        if (!(self::$instance instanceof self)) {
            self::$instance = new self($requestQuery);
        }
    ...
    public function search(&$searchArr = [], $rule = [], $filterArr = [], $filter = 'only')
    {
        ...
        $outQuery = [
            ...
            'where', 'where_in', 'where_in_sort', 'include', 'extend', 'search',
            'sort', 'time'
        ];
        //过滤排除参数
    ...
    {
        $per_page   = env('PAGE.page_size_default', 20);
        $page       = 1;
    ...
    public function pageParamFormat(&$per_page, &$page, $pageAble = true)
    {
        if ($pageAble) {
            ...
            if ($this->getQuery('page_size')) {
                $per_page = (int)$this->getQuery('page_size', 20);
            } else if ($this->getQuery('_perpage')) {
                $per_page = (int)$this->getQuery('_perpage', 20);
            }

            if ($this->getQuery('page')) {
                $page = (int)$this->getQuery('page', 1);
            } else if ($this->getQuery('_page')) {
